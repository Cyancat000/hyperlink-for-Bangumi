...
`
  ...
  //===AniDB部分===
  const renderAniDB = () => {
    fetch('https://anidb.net/perl-bin/animedb.pl?show=json&action=search&type=anime&query=' + nameSingle, {
      // fetch('https://anidb.rotcool.me/api/s?content='+nameSingle, {
      method: 'get',
      headers: {
        'Content-Type': 'application/json',
        mode: 'no-cors',
...
.rating-card .setting-card input{
  position: relative;
  top: 2px;
}
